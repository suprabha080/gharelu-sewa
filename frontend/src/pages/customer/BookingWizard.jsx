...
import { Wrench, Calendar, MapPin, CreditCard, ChevronRight, X, Zap, Wind, Sparkles } from 'lucide-react';
...
export default function BookingWizard() {
  ...
  const categoryParam = queryParams.get('category') || 'Plumbing';
  ...
  const categoryData = servicesByCategory[categoryParam] || servicesByCategory['Plumbing'];
    ...
    if (currentStep < 4) {
      ...
      try {
        ...
        const res = await bookingAPI.createBooking({
          ...
          location: address,
          description: `${categoryParam} - ${serviceObj?.name || selectedService}: ${notes || 'No notes'}`,
          is_emergency: false
        });
        setIsLoading(false);
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
        ...
        <div className="px-6 py-6 border-b border-gray-50 flex justify-between items-start bg-gray-50/20">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Category: <span className="font-bold text-[#07535f]">{categoryParam}</span></p>
          </div>
          <button
            onClick={() => navigate('/')}
        ...
        <div className="p-8">
          {currentStep === 1 && (
            <div>
              ...
              <p className="text-xs text-gray-400 mb-6">Choose the service you need from Rajesh Plumbing Works</p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        {s.est && <span className="text-[10px] text-gray-400 font-semibold block">est.</span>}
                      </div>
                    </label>
                  );
                })}
          ...
          {currentStep === 2 && (
            <div>
              <h2 className="text-lg font-bold text-gray-800 mb-1">Schedule Appointment</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-xl text-xs text-gray-500">
                  ⚡ Rajesh Plumbing Works usually responds to booking requests within 15 minutes.
                </div>
              </div>
            </div>
          )}
          ...
          {currentStep === 4 && (
            <div>
              <h2 className="text-lg font-bold text-gray-800 mb-1">Confirm & Pay</h2>
